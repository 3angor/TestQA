...
// Делаем экспорт шаг в библиотеке
@ТипШага: Загрузка данных заказа (константы)
@Описание: Экспортный сценарий который заполняет данные для заказа (константы)
@ПримерИспользования: И заполнение данных констант
